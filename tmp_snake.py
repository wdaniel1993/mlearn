"""Test sequence snake pill-badge variant: colors + labels, no auto-numbers?"""
import re, sys
...
SNAKE = """infographic sequence-snake-steps-pill-badge
data
...
svg, err = render_spec(SNAKE, TOOLS)
if svg is None:
    print("RENDER FAILED:", err)
    sys.exit(0)
Path("/tmp/snake_pill.svg").write_text(svg)
